...
class Rpc : public RpcWriterInterface<ResponseType> {
 public:
  ...
  void OnConnection() override {
    if (!handler_) {
      ...
      // FIXME: Here is where I need to downcast (this) to the writer type
  ...
  void RequestNextMethodInvocation() override {
    ...
    switch (rpc_handler_info_.rpc_type) {
      case ::grpc::internal::RpcMethod::BIDI_STREAMING:
        service_->DoRequestAsyncBidiStreaming(
      ...
      case ::grpc::internal::RpcMethod::CLIENT_STREAMING:
        service_->DoRequestAsyncClientStreaming(
            method_index_, &server_context_, streaming_interface(),
            server_completion_queue_, server_completion_queue_,
            GetRpcEvent(Event::NEW_CONNECTION));
        break;
  ...
  void HandleSendQueue() override {
    ...
    if (!send_item.msg ||
      ...
      PerformFinish(send_item.msg, send_item.status);
      return;
    }
  ...
  void Write(flatbuffers::grpc::Message<ResponseType> message) override {
